...
// Get all Bookings for a Spot based on the Spot's id
  router.get('/:spotId/bookings', requireAuth, async(req, res) => {
    ...
    // If you do not having booking
    if(!booking) {
      return res.status(404).json({message: 'The user does not have booking'});
    }
  ...
  router.post('/:spotId/bookings', requireAuth, bookingValidationRules, async(req, res) => {
    const { spotId } = req.params;
    const { user } = req;
  ...
  router.get('/:spotId/reviews', async(req, res) => {
      ...
      try {
        ...
        const review = await Review.findAll({where: { spotId }, 
            attributes: [
                'id',
                'userId',
                'spotId',
  ...
  router.post('/:spotId/reviews', requireAuth, reviewValidationRules, async(req, res, next) => {
    const { spotId } = req.params;
    const { user } = req;
  ...
  router.post('/:spotId/images', requireAuth, async(req, res, next ) => {
    ...
    try {
        ...
        console.error(e);
        return res.status(500).json({error: 'An error occured while fetching spots'})
    }
  })
  ...
  router.get('/:spotId', async(req, res, next)=> {
    ...
    // Edit a Spot
    router.put('/:spotId', requireAuth, spotValidationRules, async(req, res, next) => {
      const { user } = req;
      const { spotId } = req.params;
